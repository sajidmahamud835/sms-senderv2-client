...
const Sms = () => {
	...
	const [targetedNumberList, setTargetedNumberList] = useState([]);
	const [dataChanged, setDataChanged] = useState(false);
	const [numbersList, setNumbersList] = useState("");

	useEffect(() => {
		let data = "";
		for (let index = 0; index < targetedNumberList.length; index++) {
			const element = targetedNumberList[index];
			if (index < targetedNumberList.length - 1) {
				data = data + `${element.mobile}\n`;
			} else {
				data = data + `${element.mobile}`;
			}
		}
		setNumbersList(data);
	}, [numbersList, targetedNumberList]);
						...
						<form onSubmit={handleSmsSubmit}>
							...
							<div className="my-4 d-flex flex-lg-row flex-column justify-content-between my-3">
								...
								<div className="d-flex align-items-center">
									{!isSingle && (
										<AddListNumber
											setTargetedNumberList={setTargetedNumberList}
							...
							<div className="my-4 d-flex justify-content-between flex-lg-row flex-column my-3">
								...
								>
									...
									{myNumbers?.map((myNumber) => (
										<option key={myNumber?._id} value={myNumber?.number}>
											{myNumber?.number}
										</option>
									))}
								</select>
							...
							{isAdvanced && (
								<div className="my-4 ">
									<div className="d-flex justify-content-between align-items-center mb-3">
										<label htmlFor="delay" className="w-50">
											Delay:
										</label>
										<input
											id="delay"
									...
									<div className="d-flex justify-content-between align-items-center my-3">
										<label htmlFor="timeout" className="w-50">
											Timeout:
										</label>
										<input
											id="timeout"
